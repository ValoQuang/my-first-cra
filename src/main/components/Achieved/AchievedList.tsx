...
import { handleEditTask, handleDeleteTask } from "../../../redux/task/taskSlice";

const AchievedList = () => {
  const { tasks, searchResult } = useSelector((state: StoreState) => state.task);
  ...
  const renderTasks = useCallback(() => {
    return currentTasks.map((task, index) => (
      <tr
        className="py-1 hover:cursor-pointer hover:transition-all hover:translate-x-1 bg-base-200"
        key={index}
      >
        <td className="w-32">{task.title}</td>
        <td className="w-32">{task.message}</td>
        <td className="w-24">{task.datetime}</td>
        <th className="w-20 flex gap-1">
          <Button
            onClick={() => handleDeleteAchieved(task.id!)}
            icon={<LuClipboardEdit />}
          />
          <Button onClick={() => handleDeleteAchieved(task.id!)} icon={<LuTrash2 />} />
        </th>
      </tr>
    ));
